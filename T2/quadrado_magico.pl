...
ver_linhas(e(_,[v(p(X,_), _, V)|R]),S, T):-
...
ver_colunas(e(_,[v(p(_,Y), _, V)|R]),S, T):-
...
ver_diagonal_dir(e(_,[v(p(X,Y), _, V)|R]),S, T):-
    ...
    (=(Tc,Ta)
        ->
        (verify_dir([(X,Y,V)|L], _), diag(Diag), 
        sum_list(Diag,S), length(Diag,T))
    ;
    true).


ver_diagonal_esq(e(_,[v(p(X,Y), _, V)|R]),S, T):-
    tamanho_tabuleiro(Tb),
    Ta is Tb * Tb,
    findall((K1,K2,V1),member(v(p(K1,K2),_,V1),R),L),
    length([(X,Y,V)|L],Tc),
    (=(Tc,Ta)
        ->
        (verify_esq([(X,Y,V)|L], _), diag2(Diag),
        sum_list(Diag,S), length(Diag,T))
    ;
    true).
...
/*** verificações auciliares ***/

verify_dir([],[X, Y, Z, W, J|_]):-
    tamanho_tabuleiro(K),
    ((K=3 ) 
